...
public class LR0State {
    ...
    private void closure(Grammar grammar) {
        ...
        do {
            changeFlag = false;
            LinkedHashSet<LR0Item> temp = new LinkedHashSet<>();
            for (LR0Item item : items) {

                if (item.getCurrentTerminal()!=null && grammar.isVariable(item.getCurrentTerminal())) {
                    LinkedHashSet<Rule> rules = grammar.getRuledByLeftVariable(item.getCurrentTerminal());
    ...
    private HashSet<LR0Item> createLR0Item(HashSet<Rule> rules) {
        LinkedHashSet<LR0Item> results = new LinkedHashSet<>();
        for (Rule rule : rules) {
